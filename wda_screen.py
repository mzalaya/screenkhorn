...
def wda_sinkhorn(X, y, p=2, reg=1, k=10, solver=None, maxiter=100, verbose=0, P0=None):
    ...
    def cost(P):
        ...
        for i, xi in enumerate(xc):
            xi = np.dot(xi, P)
            if type(xi).__module__ == 'autograd.numpy.numpy_boxes':
                type(xi).__module__ = 'numpy'
            for j, xj in enumerate(xc[i:]):
                xj = np.dot(xj, P)
                if type(xj).__module__ == 'autograd.numpy.numpy_boxes':
                    type(xj).__module__ = 'numpy'
...
def wda_screenkhorn(X, y, p=2, reg=1, k=10, solver=None, maxiter=100, verbose=1, P0=None):
    # noqa
    mx = np.mean(X)
    X -= mx.reshape((1, -1))
    ...
    def cost(P):
        ...
        for i, xi in enumerate(xc):
            xi = np.dot(xi, P)
            # if type(xi) is not 'numpy.ndarray':
            ...
            for j, xj in enumerate(xc[i:]):
                xj = np.dot(xj, P)
                # print("xj", type(xj))
                # if type(xj) is not 'numpy.ndarray':
                    # print("xj is here")
                    # type(xj) = 'numpy.ndarray'


                M = dist(xi, xj)
                # if isinstance(M, np.ndarray) == False:
                    # M = M._value
                screenkhornWDA = Screenkhorn(wc[i], wc[j + i], M, reg, 30, 30)
