...
def compare_marginals(a, b, M, reg, decvect=[50, 10, 5, 2, 1.1]):
    ...
    for j, p in enumerate(decvect):
        print('p:', p)
        # screenkhorn
        ns_budget = int(np.ceil(a.shape[0] / p))
        nt_budget = int(np.ceil(b.shape[0] / p))

        tic = time()
        screenkhorn = Screenkhorn(a, b, M, reg, ns_budget, nt_budget, verbose=False)
        P_sc = screenkhorn.lbfgsb()
        time_bfgs = time() - tic
