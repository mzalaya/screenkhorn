...
\caption{Plots of $\varepsilon$ and $\kappa$ as a functions of number budget of points for a screening test with $n_b=m_b$ and the parameters $\mu, \nu, \eta, C$ are setting as in Figure~\eqref{fig:motivations}. As can be seen, $(\varepsilon, \kappa)$ tends to $(0,1)$ as $(n_b,m_b)$ tends to $(n,m)$.}
...
LBFGS handles box constraints on variables, and it becomes more efficient when these box bounds are carefully determined for problem~\eqref{screen-sinkhorn_second_def}. 
...
\noindent \textbf{Step 2:} \textcolor{black}{LBFGS}\\
...
\nl   $\theta \gets \text{LBFGS}(\theta^{(0)}, \underline{\theta}, \bar{\theta});$\\
