...
\vspace{-15pt}
\centering
\includegraphics[width=0.4\textwidth]{./figs/motivations.pdf}%, height=0.2\textwidth
\caption{Plotting of $e^{u^\star}, e^{v^\star}$ solution of dual of Sinkhorn divergence~\ref{sinkhorn-dual}.}
\label{fig:motivations}
\vspace{-15pt}
\end{wrapfigure}

\paragraph{Motivation.} The key idea of our approach comes from the so-called \emph{static screening test}~\cite{Ghaoui2010SafeFE} in supervised learning, a method is able to quickly identify inactive features, i.e., features that have zero components in the solution vector. 
...
\paragraph{Static screening test.} The core of the static screening test developed in this work aims at locating two subsets of indices $(I, J)$ in $\{1, \ldots, n\}\times\{1, \ldots, m\}$ satisfying: $e^{u_i}\geq \alpha_u, \text{ and } e^{v_j}\geq \alpha_v, \text{ for all } (i,j) \in I \times J$ and 
...
The sets $I$ and $J$ will be called active sets, we further emphasize that they are determined in an initialization step in the proposed algorithm before running the L-BFGS-B solver.
...
Our screening test relies on a new formulation of problem~\eqref{sinkhorn-dual}, that we refer as to \emph{approximate dual of Sinkhorn divergence}, given by:

% To construct our appropriate static screening test, the parameters $\alpha_u$ and $\alpha_v$ are expressed via a fixed numeric constants $\varepsilon > 0$ and $\kappa > 0$ to be given later. 
% In spirit of To construct the screening test, we define a new form
...
The $\kappa$-parameter plays a role of scaling factor, allowing to get a closed order of the potential variables $e^u$ and $e^v$, while the $\varepsilon$-parameter acts like a threshold for these variables.
...
As explained above, our static screening test is based on constructing two {active sets} denoted $I_{\varepsilon, \kappa}$ and $J_{\varepsilon, \kappa}$ throughout the dual problem of~\eqref{screen-sinkhorn} as follows: %depended on $\varepsilon, \kappa$ and
...
In the sequel, we denote by $n_b \in\{1, \ldots, n\}$ and $m_b\in\{1, \ldots, m\}$ the number budget of points to be given for resolving problem~\eqref{screen-sinkhorn}. More precisely, we change the domain of the objective function $\Psi_\kappa$ from $\R^n \times \R^m$ to $\R^{n_b} \times \R^{m_b}$. %\textbf{dom}\Psi_\kappa = 

\begin{wrapfigure}{o}{0.44\textwidth}
\vspace{-15pt}
\centering
\includegraphics[width=0.4\textwidth]{./figs/kappa_epsilon.pdf}%, height=0.2\textwidth
\caption{Convergence of $(\varepsilon, \kappa)$ to $(0,1)$ as $(n_b,m_b)$ tends to $(n,m)$ for the same setting as in Figure~\eqref{fig:motivations}.}
...
\end{equation}
Note that in one hand we have $|I_{\varepsilon, \kappa}| = n_b$ and $|J_{\varepsilon, \kappa}| = m_b$ by construction. On the other hand, $(n_b,m_b)$ tends to the full number budget of points $(n,m)$, and the couple parameters $(\varepsilon, \kappa)$ tends to $(0,1)$ (see Figure~\ref{fig:kappa_epsilon}), which corresponds to the dual of Sinkhorn divergence~\eqref{sinkhorn-dual}.
...
The following lemma (proof in supplementary material) expresses these bounds that are pre-calculated in the initialization step of Screenkhorn.
