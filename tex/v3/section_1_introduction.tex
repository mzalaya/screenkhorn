...
The proposed approach relies on reformulating the dual of Sinkhorn divergence by adding new constraints feasibility, that are defined through a convex set which depends on two parameters acting like threshold and scaling factor.
...
The Screenkhorn algorithm developed in this paper relies on two steps; the first one consists of an initialization step devoted to determine the active sets while the second is a constrained L-BFGS-B solver, a limited-memory quasi-Newton algorithm~\cite{nocedal1980,nocedal2006numerical}. 
