...
\begin{figure*}[t]
	...
	\end{center}
	\caption{Empirical evaluation of \emph{Screenkhorn} vs Sinkhorn for normalized cost matrix \emph{i.e.} $\norm{C}_\infty=1$. Top panel: marginal violations in relation with the budget of points. Bottom panel: (left) ratio of computation times    $\frac{T_{\text{Sinkhorn}}}{T_{\text{\emph{Screenkhorn}}}}$ and, (right) relative divergence variation. The results are averaged over $30$ trials.} 
...
reasonable approximation of the Sinkhorn divergence. As such, we believe that  \emph{Screenkhorn} will be practically useful in cases where modest accuracy on the divergence is sufficient. This may be the case of a loss function for a gradient descent method (see next section).
...
\begin{figure*}[t]
	...
	\caption{OT Domain adaptation : running time gain  for MNIST as a function of the number of examples and the data decimation factor \emph{Screenkhorn}. Hyperparameter values (left) $1$. (right) $10$.}
	\label{fig:otda}
\end{figure*}
...
for handling the non-convexity of the problem. Hence, at each iteration, a Sinkhorn/\emph{Screenkhorn} has to be computed and the number of iteration is
