...
In our implementation, we choose a clever starting point of the L-BFGS-B algorithm based on a method, called \emph{restricted Sinkhorn} (see Algorithm~\ref{restricted_sinkhorn}), which is a Sinkhorn algorithm applied on
the active dual variables. While simple and efficient the solution of this 
\emph{restricted Sinkhorn} algorithm does not satisfy the lower bound constraints of Problem \eqref{screen-sinkhorn_second_def}.

We further  not that, as for the \textsc{Sinkhorn} algorithm, our \textsc{Screenkhorn} algorithm can be accelerated using a GPU implementation\footnote{\url{https://github.com/nepluno/lbfgsb-gpu}} of the L-BFGS-B algorithm~\cite{fei2014-GPU-L-BFGS-B}.
...
% \nl $\bar{f}_u = \varepsilon\kappa\, c(K_{I_{\varepsilon, \kappa}, J^\complement_{\varepsilon, \kappa}});$\\
% \nl $\bar{f}_v  = \varepsilon\kappa^{-1} \,r(K_{I^\complement_{\varepsilon, \kappa}, J_{\varepsilon, \kappa}});$\\
\nl \For{$t=1,2,3$}{
	$f^{(t)}_v \gets K^\top_{I_{\varepsilon, \kappa}, J_{\varepsilon, \kappa}} u + \bar{f}_v;$\\
	$v^{(t)} \gets \frac{\nu_{J_{\varepsilon, \kappa}}}{\kappa f^{(t)}_v} ;$\\% / (\kappa K{IJ}_v);$\\
	$f^{(t)}_u \gets K_{I_{\varepsilon, \kappa}, J_{\varepsilon, \kappa}} v + \bar{f}_u;$\\
	$u^{(t)} \gets \frac{\kappa \mu_{I_{\varepsilon, \kappa}}}{f^{(t)}_u};$\\
	$u \gets u^{(t)}$, $v \gets v^{(t)}$;
}
\nl \Return{$(u^{(t)}, v^{(t)})$}
\end{algorithm}
...
\begin{figure*}[htbp]
	...
	\caption{\ar{$\frac{T_{\textsc{Greenkhorn}}}{T_{\text{\textsc{Screenkhorn}}}}$: Running time gain  for the toy problem (see Section~\ref{subsec:analysing_toy_problem}) as a function of the data decimation factor in \textsc{Screenkhorn}, for different settings of the regularization parameter $\eta$.
...
\begin{figure*}[htbp]
	...
	~\hfill~\includegraphics[width=6.5cm]{./figs/wda_accur_toy.pdf}~\hfill~
	\includegraphics[width=6.5cm]{./figs/wda_accur_mnist.pdf}~\hfill~\\
...
\begin{center}
	...
	~\hfill~\includegraphics[width=6.6cm]{./figs/wda_accur_toy_eta01_K1.pdf}~\hfill~
	\includegraphics[width=6.5cm]{./figs/wda_gain_toy_eta01_K1.pdf}~\hfill~\\
	
	~\hfill~\includegraphics[width=6.5cm]{./figs/wda_accur_toy_eta01_K5.pdf}~\hfill~
	\includegraphics[width=6.5cm]{./figs/wda_gain_toy_eta01_K5.pdf}~\hfill~\\
	
	
	
	
	\caption{(top-left) Accuracy and (bottom-right) computational time gain on the toy dataset for $\eta=0.1$ and $1$-nearest-neighbour. (bottom) accuracy and gain but for a $5$-nearest-neighbour. We can note that a  slight loss of performances occur for larger training set sizes especially for $1$-nearest-neighbour. Computational gains increase with the dataset size and
		is on average of the order of magnitude. \ar{Checker 3500 K5(plus d'iter)} 
		\label{fig:wda_gain_toy_eta01}}
...
\begin{figure*}[htbp]
	...
	\includegraphics[width=6.5cm]{./figs/da_regpath_accur_mnist_reg1.pdf}
	\includegraphics[width=6.5cm]{./figs/da_regpath_time_mnist_reg1.pdf}

	\caption{OT Domain adaptation MNIST to USPS : (left) Accuracy and (right) running time of \textsc{Sinkhorn} and \textsc{Screenkhorn} for the best performing (on average of $10$ trials) hyperparameter $\ell_{p,1}$ chosen among the set $\{0.1, 1,5,10 \}$. We can note that in this situation, there is not loss of accuracy while our \textsc{Screenkhorn} is still about an order of magnitude more efficient than Sinkhorn.}
\end{figure*}